...
function searchFile(auth, folderInfo, fileName) {
  ...
  service.files.list({
    ...
    if (file && file.id) {
      var projectResFolder = '/home/federicofarina/Workspace/WiFiesta_Android/app/src/main/res/';
      var dest = fs.createWriteStream(projectResFolder + destinyFolder + "/" + fileName);
      service.files.get({
        fileId: file.id,
        alt: 'media'
      })
